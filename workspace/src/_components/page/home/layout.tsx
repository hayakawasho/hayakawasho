...
export function Head({ children }: { children: React.ReactNode }) {
  return (
    <div
      className={
        "pointer-events-none absolute pc:sticky pc:top-0 top-0 left-0 size-full pc:h-[var(--100vh)] pc:p-[var(--gap)] px-[2.4rem] pc:pt-[4.8rem] pt-[4rem] pc:pb-[5.6rem]"
      }
    >
...
export function HeadGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="absolute pc:relative bottom-[1.6rem] pc:bottom-auto pc:block flex pc:h-full w-full items-start justify-between overflow-hidden pc:overflow-visible pc:text-right pc:text-[var(--color-text)] text-[var(--theme-text)]">
      {children}
    </div>
  );
}
