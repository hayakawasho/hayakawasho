...
export function HeadGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="absolute pc:relative bottom-0 pc:bottom-auto pc:block flex pc:h-full w-full items-start justify-between overflow-hidden pc:overflow-visible pc:text-right pc:text-[var(--color-text)] text-[var(--theme-text)]">
      {children}
    </div>
  );
}
